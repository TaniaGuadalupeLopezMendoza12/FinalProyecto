...
function cargarModeloDesdeArchivo() {
    fetch('MioEjemplo.txt')
        ...
        if (obj.read(contenido)) {
            ...
            var CENTRO_ROTOR_INDEX = 69; // Define el índice para mayor claridad
            if (obj_vertices_inmutables[CENTRO_ROTOR_INDEX] instanceof Point3D) {
                centroRotor = new Point3D(obj_vertices_inmutables[CENTRO_ROTOR_INDEX].x, obj_vertices_inmutables[CENTRO_ROTOR_INDEX].y, obj_vertices_inmutables[CENTRO_ROTOR_INDEX].z);
            }
            else {
                console.error("ERROR: El punto ".concat(CENTRO_ROTOR_INDEX, " (centroRotor) no es un Point3D v\u00E1lido o es indefinido en el archivo. La animaci\u00F3n del rotor no funcionar\u00E1."));
...
function rotarRotorAutomatico() {
    ...
    /**
     * Procesa la rotación y el pitch de una aspa específica y actualiza los vértices
    ...
    var procesarAspa = function (orig, tras, baseIx, backIx, fase) {
        ...
        if (!(pivotAspaOriginal instanceof Point3D)) {
            ...
            girando = false; // Detener la animación si hay un error
 ...
 */
function calcularNuevosVerticesAspa(verticesAspaOriginal, pitchAngle, rotorGlobalAngle, pivotAspa, localRotationAxis, globalRotationCenter, globalRotationAxis) {
    var nuevosVertices = [];
