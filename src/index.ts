...
let cv: CvHLines;
let obj: Obj3D;
let ang: number = 0; // Ángulo general 
        ...
        .then(contenido => {
            ...
            if (obj.read(contenido)) {
                ...
                if (obj_vertices_inmutables[CENTRO_ROTOR_INDEX] instanceof Point3D) {
                    ...
                    console.error(`ERROR: El punto ${CENTRO_ROTOR_INDEX} (centroRotor) no es un Point3D válido o es indefinido en el archivo. La animación del rotor no funcionará.`);
...
function calcularNuevosVerticesAspa(
    ...
    pivotAspa: Point3D,
    localRotationAxis: Point3D, 
                                
    globalRotationCenter: Point3D,
    globalRotationAxis: Point3D 
): Point3D[] {
